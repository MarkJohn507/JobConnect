...
/* ══════════════════════════════════════════════════════
   SHARED: Admin Layout (sidebar + topbar)
══════════════════════════════════════════════════════ */
...
export function AdminDashboardPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([getDocs(collection(db, 'applications')), getDocs(collection(db, 'users'))])
      .then(([aSnap, uSnap]) => {
      ...
      <div className="fade-in">
        ...
        <div className="admin-stats">
          {[
            ...
            { label: 'Offers', val: offer, color: '#a78bfa' },
          ...
          <div className="table-wrap">
            <table>
              ...
              <tbody>
                {recent.length === 0 ? (
                  <tr>
                    <td colSpan={4} style={{ textAlign: 'center', padding: 24, color: 'var(--text2)' }}>
                      No applications yet
                    </td>
                  </tr>
                ) : (
                  recent.map(app => (
                    <tr key={app.id}>
                      ...
                      <td style={{ whiteSpace: 'nowrap' }}>
                        {formatSafeDate(app.appliedDate)}
                      </td>
                    </tr>
                  ))
                )}
  ...
  return (
    <AdminLayout active="Users">
      <div className="fade-in">
        ...
        <p style={ps.sub}>
          {users.length} registered user{users.length !== 1 ? 's' : ''} · {users.filter(u => u.disabled).length} deactivated
        </p>
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          <div className="table-wrap">
            <table>
              <thead>
                <tr>
                  {['Name', 'Email', 'Date Joined', 'Status', 'Action'].map(h => <th key={h}>{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={5} style={{ textAlign: 'center', padding: 32, color: 'var(--text2)' }}>
                      No users found
                    </td>
                  </tr>
                ) : (
                    ...
                    return (
                      <tr key={user.id}>
                        <td style={{ fontWeight: 600 }}>
                          {user.displayName || <span style={{ color: 'var(--text2)' }}>—</span>}
                        </td>
                        <td>{user.email}</td>
                        <td style={{ whiteSpace: 'nowrap' }}>
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          <div className="table-wrap">
            <table>
              <thead>
                <tr>
                  {['User ID', 'Company', 'Position', 'Status', 'Applied Date'].map(h => <th key={h}>{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {filtered.length === 0 ? (
...
export function AdminAnalyticsPage() {
  ...
  const successRate = total ? ((offer / total) * 100).toFixed(1) : '0.0'
  ...
  const STATUS_BARS = [
    ...
    { label: 'Offer', count: offer, color: '#fbbf24' },
      ...
      <div className="fade-in">
        ...
        <div className="admin-stats">
          {[
            ...
            { label: 'Total Apps', val: total, color: '#60a5fa' },
            { label: 'Offer Rate', val: `${successRate}%`, color: '#fbbf24' },
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ height: 5, background: 'var(--surface2)', borderRadius: 3, marginTop: 4, overflow: 'hidden' }}>
                      <div
                        style={{
                          width: `${(count / topCompanies[0][1]) * 100}%`,
                          height: '100%',
                          background: 'var(--accent)',
                          borderRadius: 3,
                        }}
